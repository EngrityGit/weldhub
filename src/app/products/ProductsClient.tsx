...
import { getFilterOptions, filterAndSortProducts, computeDisplayPrice } from '@/lib/products'
...
export default function ProductsPage() {
  ...
  }, [searchQuery, selectedCategories, selectedBrands, selectedConditions, onSaleOnly, priceRange, sortBy])
                          ...
                          <tr key={product.id}>
                            ...
                            <td className="hidden md:table-cell">
                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                                ...
                                product.condition === 'Open Box' ? 'bg-sky-100 text-sky-700' :
                                'bg-gray-100 text-gray-600'
                              }`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${
                                  product.condition === 'New'      ? 'bg-emerald-500' :
                                  product.condition === 'Used'     ? 'bg-amber-500' :
                                  product.condition === 'Open Box' ? 'bg-sky-500' :
                                  'bg-gray-400'
                                }`} />
                                {product.condition}
                              </span>
                            ...
                            <td className="text-right">
                              <div className={`font-bold text-sm ${isDiscounted ? 'text-red-600' : 'text-gray-900'}`}>
                                {displayPrice}
                              </div>
                              ...
                              {product.bulkPricing && (
                                <div className="text-[10px] text-emerald-600 font-semibold mt-0.5">
                                  {product.bulkPricing.minQty}+ units: {product.bulkPricing.pricePerUnit}
                                </div>
                              )}
                            </td>
